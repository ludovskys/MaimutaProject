...
public class DMSTestActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		customDrawableViewMain = new CustomDrawableView(this);
		...
		customDrawableViewMain.setIsTheMainView(true);
		customDrawableViewMain.setIsLikeTheMainView(false);
		...
		customDrawableViewMain.setOnTouchListener(new OnTouchListener()
		{
			@Override
			...
			view.setOnTouchListener(new OnTouchListener()
			{
				...
				public boolean onTouch(View v, MotionEvent event) {
					...
					h.post(new Runnable() 
					{
						public void run() 
						{
							// if the view chosen is like the main view, we win !
							if (view.getIsLikeTheMainView())
							{
								...
								Test t = new Test(numberOfTrials, numberOfChoices, phase1Delay, delayBetweenPhase1And2, phase2Delay, numberOfMistakes, SystemUtils.DMS_TEST);
	...
	private void initNewTrial()
	{
		...
		customDrawableViewMain.setCustomView(new CustomDrawableView(getApplicationContext(), intColor, width, height));
		// width and height
		layoutParams = new RelativeLayout.LayoutParams(width, height);
		// set position
		layoutParams.setMargins(screenWidth/2-width/2,screenHeight/8,0,0);
		customDrawableViewMain.setLayoutParams(layoutParams);
		...
		for (int i= 0; i < numberOfChoices; i++)
		{
			...
			// one of these view will be the same as the same view
			if (randomNumber == i)
			{
				// this one will be like the main view !
				view.setCustomView(customDrawableViewMain);
	...
	private void buildResult()
	{
		...
		String title = "DMSTest_"+dateToString;
		
		String title2 = "DMS Test : "+dateToString2;
		...
		if (isOnline)
		{
			...
			String xmlResults = BuildXML.buildXML("monkeyModule", "createTest",SystemUtils.DMS_TEST,settings.getString("userName", "Sans nom"),
					formatter.format(totalSeconds),arrayListTests);
			
			// send datas
			processExecuterModule.runSendTestData(DMSTestActivity.this,title,res,sendResultByEmail,title2,xmlResults);
		}
		else
		{
